...
exports.create = (req, res, next) => {
  ...
  if (req.file == undefined && req.body.content == "") {
    return res
      .status(400)
      .json({ message: "Veuillez ne pas envoyer de message vide." });
  } else {
    const imgUrl =
      req.file == undefined ? null : `./images/${req.file.filename}`;
    ...
    (err, result, fields) => {
      if (result[0].users_id === req.userId || hasRight === 1) {
        database.query(
          ...
          [null, req.body.content, req.params.post_id],
          (err, result, fields) => {
            if (err) {
              return res.status(400).json({ message: "Il y a eu une erreur!" });
            } else {
              return res
                .status(201)
                .json({ message: "L'image a bien été supprimée" });
            }
          }
        );
      } else {
        return res
          .status(401)
          .json({ message: "Vous n'êtes pas autorisé à supprimer l'image!" });
      }
    }
  );
};
          ...
          database.query(
            ...
            (err, result, fields) => {
              if (err) {
                ...
                return res
                  .status(200)
                  .json({ message: "Vous aimez la publication" });
              }
            }
          );
        }
